...
public record KeycloakConfig
{
    ...
    public async Task<string> GenerateLoginUrl(string redirectUrl, CancellationToken cancellationToken)
    {
        ...
        {
            KeycloakOAuthConfig? config = await response.Content.ReadFromJsonAsync<KeycloakOAuthConfig>(cancellationToken);
            if (config != null)
            {
        ...
        throw new InvalidOperationException($"Failed to deserialize Keycloak OAuth configuration: {await response.Content.ReadAsStringAsync(cancellationToken)}");
    }
}
